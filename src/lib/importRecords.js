...
export function parseCsv(text) {
  ...
  const headers = rawHeaders.map((h, i) => mapped[i] || canonicalHeader(h));
  return lines.slice(1).map((line, index) => {
    const cells = parseLine(line);
    return { ...Object.fromEntries(headers.map((h, i) => [h, cells[i] || ''])), _row: index + 2 };
  });
}
  ...
  return (
    {
      ...
      return: 'return',
      found: 'adjustment_in',
      found_extra: 'adjustment_in',
      missing: 'adjustment_out',
...
export function inferRecordType(row) {
  const explicit = String(firstValue(row, ['record_type', 'type', 'record']))
    .toLowerCase()
    .trim()
    .replace(/[- ]+/g, '_');
