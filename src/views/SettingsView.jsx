...
import { Download, Upload, ShieldCheck, Database, Sprout, NotebookPen, Type, HardDrive } from 'lucide-react';
import { getBackupHealth } from '../lib/backupHealth.js';
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        ...
        <div className="grid grid-cols-2 gap-2" role="group" aria-label="Text size">{FONT_SIZES.map((option) => <button key={option.value} type="button" aria-pressed={fontSize === option.value} onClick={() => changeFontSize(option.value)} className="rounded-xl px-3 py-3 text-left border min-h-[58px]" style={{ borderColor: fontSize === option.value ? 'var(--forest)' : 'var(--line)', background: fontSize === option.value ? 'var(--forest-tint)' : 'var(--surface)', color: 'var(--ink)' }}><span className="block font-semibold">{option.label}</span><span className="block text-xs mt-0.5" style={{ color: 'var(--ink-soft)' }}>{option.description}</span></button>)}</div>
      </section>
      <section className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}><div className="flex items-start gap-3"><HardDrive size={20} style={{ color: 'var(--forest)', marginTop: 2 }} /><div><h2 className="font-semibold">Backup health</h2><p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>{backup.needsBackup ? 'No recent backup is recorded. Download one now and keep a copy outside this browser.' : `Last backup was ${backup.backupAgeDays} day${backup.backupAgeDays === 1 ? '' : 's'} ago.`}</p></div></div><button type="button" onClick={exportData} className="btn-primary rounded-xl px-4 py-2.5 text-sm inline-flex items-center gap-2 mt-4"><Download size={16} /> Download backup</button></section>
      <section className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}><div className="flex items-start gap-3"><Sprout size={20} style={{ color: 'var(--forest)', marginTop: 2 }} /><div><h2 className="font-semibold">Learn how to use Mazaosmart</h2><p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>Take the short farm tour again. It uses a temporary example and does not change your real farm records.</p></div></div><button type="button" onClick={replayTutorial} className="btn-ghost rounded-xl px-4 py-2.5 text-sm inline-flex items-center gap-2"><Sprout size={16} /> Show me around</button></section>
      <section className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}><div className="flex items-start gap-3"><NotebookPen size={20} style={{ color: 'var(--forest)', marginTop: 2 }} /><div><h2 className="font-semibold">Bring in existing records</h2><p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>Have records in a notebook or spreadsheet? Add them without replacing the records already on this device.</p></div></div><button type="button" onClick={() => onNavigate('import-records')} className="btn-primary rounded-xl px-4 py-2.5 text-sm inline-flex items-center gap-2"><NotebookPen size={16} /> Bring in records</button></section>
