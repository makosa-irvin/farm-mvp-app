    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>
          Your farm records stay on this device unless you choose to export them.
        </p>
      </header>

      <section className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        <div className="flex items-start gap-3">
          <Type size={20} style={{ color: 'var(--forest)', marginTop: 2 }} />
          <div>
            <h2 className="font-semibold">Text size</h2>
            <p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>
              Choose a comfortable reading size. Default is slightly larger for easier reading on phones.
            </p>
          </div>
        </div>
      ...
      <section className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        <div className="flex items-start gap-3">
          <NotebookPen size={20} style={{ color: 'var(--forest)', marginTop: 2 }} />
          <div>
            <h2 className="font-semibold">Bring in existing records</h2>
            <p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>
              Have records in a notebook or spreadsheet? Add them without replacing the records already on this device.
            </p>
          </div>
        </div>
        <button
          type="button"
          onClick={() => onNavigate('import-records')}
          className="btn-primary rounded-xl px-4 py-2.5 text-sm inline-flex items-center gap-2"
        >
          <NotebookPen size={16} /> Bring in records
        </button>
      </section>
      ...
      <section className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>
              Mazaosmart has no backend account or automatic cloud sync. Exported files are controlled by you and should be stored somewhere
              safe.
            </p>
          </div>
        </div>
