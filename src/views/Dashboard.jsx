...
export default function Dashboard({ units, logs, expenses, inventory = [], inventoryMoves = [], goTo }) {
  ...
  const todaySpend = expenses
    .filter((expense) => expense.date === today && !expense.inventoryTransactionId)
  ...
  const productionByUnit = units
    .map((unit) => ({
      unit,
      produced: logs
        .filter((log) => log.unitId === unit.id && inPeriod(log.date, 'week'))
        .reduce((sum, log) => sum + (Number(log.produced) || 0), 0),
    }))
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        <div className="text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--ink-soft)' }}>
        ...
        <div className="mt-2 flex items-end justify-between gap-4">
          <div>
            <div
              ...
              style={{ color: estimatedSurplus === null ? 'var(--ink)' : estimatedSurplus >= 0 ? 'var(--forest)' : 'var(--rust)' }}
            >
          ...
          <TrendingUp
            size={24}
            style={{ color: estimatedSurplus === null ? 'var(--ink-soft)' : estimatedSurplus >= 0 ? 'var(--forest)' : 'var(--rust)' }}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          {singleTypeToday ? (
            <TodayCard
              icon={Egg}
              label="Produced"
              value={fmtNum(producedTodayByType[singleTypeToday])}
              sub={typeOf({ type: singleTypeToday }).unitLabel}
            />
          ) : (
            <TodayCard
              icon={Egg}
              label="Logged today"
              value={fmtNum(typesLoggedToday.length)}
              sub={typesLoggedToday.length === 1 ? 'kind of produce' : 'kinds of produce'}
            />
          )}
          ...
          <TodayCard
            icon={Boxes}
            label="Stock used"
            value={fmtNum(inventoryMoves.filter((move) => move.date === today && move.direction === 'out').length)}
            sub="stock movements"
          />
      ...
      <section className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        ...
        <div className="grid grid-cols-2 gap-4 mt-3">
          <div>
            ...
            <div className="font-mono text-lg font-semibold mt-1">{fmtMoney(farmCostsThisMonth)}</div>
          </div>
          <div>
            <div className="text-xs" style={{ color: 'var(--ink-soft)' }}>
              Money spent
            </div>
            <div className="font-mono text-lg font-semibold mt-1">{fmtMoney(moneySpentThisMonth)}</div>
          </div>
        </div>
        ...
        <div className="text-xs mt-3 leading-5" style={{ color: 'var(--ink-soft)' }}>
          <strong>Farm costs</strong> also counts stock the farm used or lost, even if you paid for it earlier. <strong>Money spent</strong>{' '}
          is only new cash payments made this month.
        </div>
        <button
          type="button"
      ...
      <section className="rounded-2xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => goTo('log')}
            className="btn-primary flex min-h-12 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm"
          >
          ...
          <button
            ...
            className="btn-ghost flex min-h-12 items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm"
          >
            <Plus size={16} /> Stock
          </button>
          <button
