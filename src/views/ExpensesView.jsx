...
// Expenses includes both money paid and non-cash farm costs. Buying stock is
// a cash payment and creates inventory; using/loss/spoilage later becomes a
// non-cash cost at the inventory ledger's weighted-average value. This avoids
// charging the same purchase twice while still recognizing what the farm used.
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={submit} className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        ...
        <div className="grid grid-cols-2 gap-3.5">
          <div><FieldLabel>What was it for?</FieldLabel><select value={category} onChange={(e) => setCategory(e.target.value)} className={inputClass} style={inputStyle}>{EXPENSE_CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
          <div><FieldLabel>How much did you pay? (KSh)</FieldLabel><input type="number" min="0" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0" required className={inputClass} style={inputStyle} /></div>
        </div>
        <div className="grid grid-cols-2 gap-3.5">
        ...
        <div className="grid grid-cols-2 gap-3.5">
          ...
          <div><FieldLabel>How much stock?</FieldLabel><input type="number" min="0.01" step="0.01" value={inventoryQuantity} onChange={(e) => setInventoryQuantity(e.target.value)} disabled={!inventoryItemId} required={!!inventoryItemId} placeholder="e.g. 50" className={inputClass} style={inputStyle} /></div>
        </div>
        <div><FieldLabel>Note (optional)</FieldLabel><input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="e.g. 50 kg layer mash" className={inputClass} style={inputStyle} /></div>
        <div className="grid grid-cols-2 gap-3.5">
          <div><FieldLabel>Who did you buy from? (optional)</FieldLabel><input type="text" value={supplier} onChange={(e) => setSupplier(e.target.value)} placeholder="e.g. local agrovet" className={inputClass} style={inputStyle} /></div>
          <div><FieldLabel>How did you pay? (optional)</FieldLabel><select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className={inputClass} style={inputStyle}><option value="">Not recorded</option>{PAYMENT_METHODS.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}</select></div>
        </div>
        <div className="flex gap-2 flex-wrap">
          <button type="submit" className="btn-primary rounded-xl px-5 py-2.5 text-sm flex items-center gap-2"><Save size={15} />{editingId ? 'Save changes' : 'Save'}</button>
        ...
        <div className="flex items-start gap-2 text-xs pt-1" style={{ color: 'var(--ink-soft)' }}><Info size={13} className="mt-0.5 shrink-0" /><span>When you buy stock, the money you pay is recorded now. When the stock is later used, lost, or spoiled, its value becomes a farm cost without counting another cash payment.</span></div>
      </form>

      {recent.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
          <div className="px-5 pt-4 pb-3 font-display text-lg font-semibold" style={{ borderBottom: '1px solid var(--line)' }}>Recent money &amp; costs</div>
          <div className="text-xs px-5 pb-2" style={{ color: 'var(--ink-soft)' }}>Stock used, lost, or spoiled is shown here as a farm cost. It does not mean you paid money again.</div>
            ...
            {recent.map((e) => {
              ...
              return <Fragment key={e.id}>
                ...
                {isExpanded && <tr style={{ background: 'var(--surface-alt)' }}><td colSpan={5} className="px-5 pb-3 pt-0 font-sans"><div className="rounded-xl p-3 space-y-1.5 text-xs" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
                  {e.nonCash && <div><span style={{ color: 'var(--ink-soft)' }}>What this means: </span>This is the value of stock the farm used or lost. No new cash payment was recorded.</div>}
                  {e.nonCash && <div style={{ color: 'var(--ink-soft)' }}>To correct this, go to Stock and edit or remove the update there — it will update here automatically.</div>}
                  {e.description && <div><span style={{ color: 'var(--ink-soft)' }}>Note: </span>{e.description}</div>}
                  {e.supplier && <div><span style={{ color: 'var(--ink-soft)' }}>Bought from: </span>{e.supplier}</div>}
                  ...
                  {item && <div><span style={{ color: 'var(--ink-soft)' }}>{e.nonCash ? 'Stock changed: ' : 'Stock added: '}</span>{fmtNum(e.inventoryQuantity, 2)} {item.unit} of {item.name}</div>}
