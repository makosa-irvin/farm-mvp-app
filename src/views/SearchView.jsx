...
export default function SearchView({ units = [], logs = [], expenses = [], inventory = [], goTo }) {
  ...
  const results = useMemo(() => {
    ...
    const matches = [];
    units.forEach((item) => {
      if (`${item.name} ${item.type}`.toLowerCase().includes(q))
        matches.push({ key: `unit-${item.id}`, icon: Tag, type: 'Group', title: item.name, detail: item.type, tab: 'units' });
    });
    inventory.forEach((item) => {
      if (`${item.name} ${item.category}`.toLowerCase().includes(q))
        matches.push({
          key: `stock-${item.id}`,
          icon: Boxes,
          type: 'Stock',
    ...
    expenses.forEach((item) => {
      if (`${item.description || ''} ${item.supplier || ''} ${item.category || ''}`.toLowerCase().includes(q))
        matches.push({
          key: `expense-${item.id}`,
          icon: Receipt,
          type: 'Expense',
          title: item.description || item.category,
          detail: `${fmtMoney(item.amount)} · ${item.date}`,
          tab: 'expenses',
        });
    });
    logs.forEach((item) => {
      const group = units.find((u) => u.id === item.unitId);
      if (`${group?.name || ''} ${item.notes || ''} ${item.date}`.toLowerCase().includes(q))
        matches.push({
          key: `log-${item.id}`,
          icon: ClipboardList,
          type: 'Daily log',
          title: group?.name || 'Daily log',
          detail: `${item.date} · ${fmtNum(item.produced || 0)} produced`,
          tab: 'log',
        });
    });
    ...
    <div className="space-y-5">
      <header>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--ink-soft)' }}>
          Find groups, stock items, expenses, suppliers, and daily logs from one place.
        </p>
      </header>
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search e.g. Layer House, feed, supplier..."
      ...
      <div className="space-y-2">
        {results.map(({ key, icon: Icon, type, title, detail, tab }) => (
          <button
            key={key}
            type="button"
