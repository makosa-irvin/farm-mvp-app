  ...
  return (
    <div className="space-y-6">
      <form onSubmit={submit} className="rounded-2xl p-5 space-y-4" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
        ...
        <div className="flex gap-2 flex-wrap"><button type="submit" className="btn-primary rounded-xl px-5 py-2.5 text-sm flex items-center gap-2"><Save size={15}/> {editingId ? 'Save changes' : 'Add unit'}</button>{editingId && <button type="button" onClick={resetForm} className="btn-ghost rounded-xl px-4 py-2.5 text-sm flex items-center gap-2"><X size={15}/> Cancel</button>}</div>
      </form>
          ...
          {units.map((u) => {
            ...
            return (
              <div key={u.id} className="flex items-center justify-between rounded-2xl px-5 py-3.5" style={{ background: 'var(--surface)', border: '1px solid var(--line)' }}>
                ...
                <div className="flex gap-1"><button onClick={() => editUnit(u)} className="p-1.5 rounded hover:bg-black/5" aria-label={`Edit ${u.name}`}><Pencil size={15} style={{color:'var(--ink-soft)'}}/></button><button onClick={() => onRemove(u.id)} className="p-1.5 rounded hover:bg-black/5" aria-label={`Remove ${u.name}`}>
